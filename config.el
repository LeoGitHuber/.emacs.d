...
(dolist (mode '(prog-mode-hook TeX-mode-hook))
...
(keymap-global-set "C-c q" 'vr/replace)
...
(defvar themes_chosen
  '(storybook doom-gruvbox)
  "Set for themes for dark and light mode")


(if (>= (string-to-number (substring (current-time-string) 11 13)) 17)
...
(let ((hls '(emacs-lisp-mode-hook lisp-mode-hook TeX-mode-hook)))
  (dolist (mode hls)
...
(with-eval-after-load 'rime (keymap-set rime-mode-map "C-`" 'rime-send-keybinding)
					  ;; (define-key rime-mode-map (kbd "Shift") 'rime-send-keybinding)
					  (define-key rime-mode-map (kbd "M-t") 'rime-inline-ascii)
...
;; (set-face-attribute 'minibuffer-prompt nil :font (font-spec :name "PragmataPro" :weight 'normal :size 12.0))
;; (set-face-attribute 'vertico-current nil :font (font-spec :name "PragmataPro" :weight 'normal :size 12.0))
;; (set-face-attribute 'vertico-multiline nil :font (font-spec :name "PragmataPro" :weight 'normal :size 12.0))
...
(defface input-method-indicator-en-face
  '((((background dark)) :foreground "#b16286")
    (((background light)) :foreground "#8f3f71"))
  "Face for personal mode line setting area 2"
  :group 'mode-line-personal)
   ...
   (:eval
	(concat
	 ...
	 (propertize (if (or (buffer-file-name) (all-the-icons-auto-mode-match?))
					 (if (equal (car custom-enabled-themes) (car (cdr themes_chosen)))
										   ...
										   (if (and (rime--should-enable-p)
											   ...
											   (concat
												...
												(propertize (format "%s-CN" (rime-lighter))
															'face `(:weight bold :inherit input-method-indicator-cn-face)
															'display '(raise 0.13)))
											 (concat
											  (propertize " | " 'face `(:inherit mode-line-seperator :weight bold) 'display '(raise 0.1))
											  (propertize (format "%s-EN" (rime-lighter))
														  'face `(:weight bold :inherit input-method-indicator-en-face)
														  'display '(raise 0.13)))
											 ))
									   (propertize " | " 'face `(:inherit mode-line-seperator :weight bold) 'display '(raise 0.1))
